...
if(isset($_POST['Select'])){
    $select= $_POST['r_names'];
    $i_id= $_POST['i_id'];
    $i_no= $_POST['i_no'];
    $sql1 ="SELECT * FROM tb_rackmeta WHERE r_name='$select'";
    $rackmeta=mysqli_query($conn,$sql1);
    echo '<legend class="text-center">Assign '.$i_no.' on Rack - '.$select.'</legend>';
    require 'rackassigntb.php';
}
else{
   // $sql2 ="SELECT * FROM tb_item ";
